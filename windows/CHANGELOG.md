...
### Fixed
- **Recorded audio was crackly and broke up** — the shared-timeline audio path re-derived every ~10 ms WASAPI packet's position from its (frame-rounded, jittery) QPC timestamp and reconciled it against the running write cursor, inserting or dropping a sample or two on every packet (~100 micro-edits per second). The audio reader was also paced to the recording clock with no allowance for WASAPI capture latency, so each read slightly outran capture, the mix buffer underran, and `ReadFully` spliced in silence. Only the first packet from each source is now aligned to the shared origin (preserving inter-source start offsets for sync) and later packets are appended contiguously, and the reader is held a fixed safety margin behind the recording clock so the buffer never underruns.
